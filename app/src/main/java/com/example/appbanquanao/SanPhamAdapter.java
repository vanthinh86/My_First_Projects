...
public  class SanPhamAdapter extends BaseAdapter {
    ...
    public View getViewWith8Properties(int i, View view, ViewGroup parent) {
        ...
        if (tt.getSoluongkho() == 0) {
            ...
            viewtemp.setOnClickListener(v -> Toast.makeText(context, "Sản phẩm đã hết hàng", Toast.LENGTH_SHORT).show());
    ...
    private void showEditDialog(SanPham tt) {
        // Đặt lại URI ảnh đã chọn để tránh lỗi logic
        ...
        // Khởi tạo các trường EditText và Spinner
        ...
        // Tải danh sách nhóm sản phẩm và điền dữ liệu
        loadAndSetNhomSanPham(mansp, tt.getMansp());
    ...
    private void updateSanPham(SanPham tt, EditText editMasp, EditText editTensp, EditText editDongia, EditText editMota, EditText editGhichu, EditText editSoluongkho, Spinner editMansp) {
        ...
        // Cập nhật ảnh nếu có
        byte[] newAnh = selectedImageUri != null ? getBytesFromUri(selectedImageUri) : tt.getAnh();
        ...
        try {
            ...
            if (newAnh != null && newAnh.length > 0) {
    ...
    // Phương thức để tải và thiết lập nhóm sản phẩm cho Spinner
    private void loadAndSetNhomSanPham(Spinner mansp, String currentMansp) {
        ApiCaller.callApi("nhomsanpham", "GET", null, response -> {
            ((Activity) context).runOnUiThread(() -> {
                if (response != null) {
                    try {
                        // Khai báo mangNSPList như một biến cục bộ để tránh lỗi
                        ArrayList<NhomSanPham> mangNSPList = new ArrayList<>();
